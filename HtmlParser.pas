...
type
    ...
    TEnumAttributeNameCallBack = function(AParam: Pointer; const AttributeName, AttributeValue: WideString): Boolean; stdcall;

    TFilterElementCallBack = function(AParam: Pointer; AElement: IHtmlElement): Boolean; stdcall;
...
type
    ...
    THtmlElement = class(TInterfacedObject, IHtmlElement, IHtmlElementObj)
        ...
        constructor CreateAsText(AOwner: THtmlElement; AText: string; ALine, ACol: Integer);
        constructor CreateAsScript(AOwner: THtmlElement; AText: string; ALine, ACol: Integer);
        constructor CreateAsStyle(AOwner: THtmlElement; AText: string; ALine, ACol: Integer);
        constructor CreateAsComment(AOwner: THtmlElement; AText: string; ALine, ACol: Integer);
        constructor CreateAsTag(AOwner: THtmlElement; AText: string; ALine, ACol: Integer);
        constructor CreateAsDocType(AOwner: THtmlElement; AText: string; ALine, ACol: Integer);
        destructor Destroy; override;
        ...
        function _FilterElement(const ATagName: WideString; const AAttributes: TAttributeDynArray): Boolean;
        procedure _FindElements(AList: TIHtmlElementList; const ATagName: WideString; const AAttributes: TAttributeDynArray; AOnlyInTopElement: Boolean);
...
begin
    Result := E.FAttributes.ContainsKey(Item.Key) and (E.FAttributes.ValueOf(Item.Key) = Item.Value);
end;
...
begin
    ...
    Value := Item.Value;
    Result := (Length(attr) > Length(Value)) and (Copy(attr, 1, Length(Value)) = Value);
end;
...
begin
    ...
    attr := E.FAttributes.ValueOf(Item.Key);
    Value := Item.Value;
    Result := (Length(attr) > Length(Value)) and (Copy(attr, Length(attr) - Length(Value) + 1, Length(attr)) = Value);
end;
...
begin
    ...
    begin
        ...
        case ElementType of
            EtUnknow:
                begin
                    DoError('LineNum:' + IntToStr(BeginLineNum) + '无法解析的内容:' + Copy(oldP, 1, 100));
                end;
            EtDocType:
                begin
                    Tag := THtmlElement.CreateAsDocType(nil, Tmp, BeginLineNum, BeginColNum);
            ...
            EtTag:
                begin
                    ...
                    begin
                        ...
                        Tag := THtmlElement.CreateAsScript(nil, Tmp, BeginLineNum, BeginColNum);
                        AElementList.Add(Tag);
                    end
                    ...
                    begin
                        ...
                        Tag := THtmlElement.CreateAsStyle(nil, Tmp, BeginLineNum, BeginColNum);
                        AElementList.Add(Tag);
                    end;
                end;
            EtText:
            ...
            EtComment:
                begin
                    Tag := THtmlElement.CreateAsComment(nil, Tmp, BeginLineNum, BeginColNum);
                    Tag.FSourceLine := BeginLineNum;
                    Tag.FSourceCol := BeginColNum;
                    AElementList.Add(Tag);
                end;
...
constructor THtmlElement.CreateAsComment(AOwner: THtmlElement; AText: string; ALine, ACol: Integer);
begin
...
constructor THtmlElement.CreateAsDocType(AOwner: THtmlElement; AText: string; ALine, ACol: Integer);
begin
    Create(AOwner, AText, ALine, ACol);
    FContent := ConvertEntities(AText);
    FTagName := '#DOCTYPE';
...
constructor THtmlElement.CreateAsScript(AOwner: THtmlElement; AText: string; ALine, ACol: Integer);
begin
    Create(AOwner, AText, ALine, ACol);
    FContent := ConvertEntities(AText);
    FTagName := '#SCRIPT';
...
constructor THtmlElement.CreateAsStyle(AOwner: THtmlElement; AText: string; ALine, ACol: Integer);
begin
    Create(AOwner, AText, ALine, ACol);
    FContent := ConvertEntities(AText);
    FTagName := '#STYLE';
...
constructor THtmlElement.CreateAsTag(AOwner: THtmlElement; AText: string; ALine, ACol: Integer);
var
    I: Integer;
...
constructor THtmlElement.CreateAsText(AOwner: THtmlElement; AText: string; ALine, ACol: Integer);
begin
    Create(AOwner, AText, ALine, ACol);
    FContent := ConvertEntities(AText);
    FTagName := '#TEXT';
...
procedure THtmlElement.EnumAttributeNames(AParam: Pointer; ACallBack: TEnumAttributeNameCallBack);
var
    I: Integer;
...
function THtmlElement._FilterElement(const ATagName: WideString; const AAttributes: TAttributeDynArray): Boolean;
var
    I: Integer;
begin
    Result := false;
    ...
    begin
        for I := Low(AAttributes) to High(AAttributes) do
            if (not FAttributes.ContainsKey(LowerCase(AAttributes[I].Key))) or (AAttributes[I].Value <> FAttributes[LowerCase(AAttributes[I].Key)]) then
                Exit;
        Result := True;
    end;
end;

procedure THtmlElement._FindElements(AList: TIHtmlElementList; const ATagName: WideString; const AAttributes: TAttributeDynArray; AOnlyInTopElement: Boolean);
var
    I: Integer;
    C: IHtmlElement;
    E: THtmlElement;
...
function THtmlElement.FindElements(ATagName: WideString; AAttributes: WideString; AOnlyInTopElement: Boolean): IHtmlElementList;
var
            ...
            case P^of
                ...
                '.': // class
                    begin
                        ...
                        pAttr^.Value := ReadStr(P, (WhiteSpace + OperatorChar + ['[', ']', '"', '''', ',', '.', '#', #0]));
                    end;
                '#': // id
                    begin
                        IncSrc(P);
                ...
                begin
                    Item.szTag := UpperCase(ReadStr(P, (WhiteSpace + ['[', ']', '"', '''', ',', '.', '#', #0])));
                end;
            end;
        end;
    end;
